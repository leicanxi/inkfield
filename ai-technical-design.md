...
截止型目标还必须通过累计截止校验。执行周从当前上海业务日期、预备周从 week_start 统计有效剩余日；后端取 `floor(allocatable_minutes × 剩余有效日数 / 通常可用日数)` 与 `allocatable_minutes - 已消耗容量` 的较小值。单任务已消耗量优先使用累计 actual_minutes；缺失且任务曾完成时回退 estimated_minutes，否则为 0。对周内每个截止日期验证所有项目 `effective_due_date <= cutoff` 的 required 任务预计分钟之和不超过该容量。考试类不计 target_date 当天，交付类可计当天；所有字段只使用 date，不引入每日任务计划。
