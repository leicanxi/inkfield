...
容量以预计分钟规划，Task.actual_minutes 保存用户明确记录的累计总投入；完成事件和后续补记都覆盖这一唯一有效投影，不按事件相加。无实际记录但曾完成时才回退 estimated_minutes；reopened 不抹掉投入，部分投入后 deferred 时旧周保留实际量、新任务只规划剩余量。完成状态、实际耗时和“轻松/合适/吃力”反馈分开保存；不引入虚拟能量条或不透明消耗值。
...
用户确认目标级信息
  ↓
按残周规则锁定 UserWeekCapacity：本周剩余不少于 4 天时锁定当前周和下一周；少于 4 天时额外锁定下下周
  ↓
为上述各周创建或调整该项目的 UserWeekAllocation
  ↓
领域服务在项目预算内事务性创建阶段、里程碑和周计划：常规情形创建当前执行周与下一预备周；残周少于 4 天时，创建本周 1—2 个启动任务、下一完整执行周和后一预备周
```
...
创建 rollover UserWeekRun，幂等键为 user_id + week_start + rollover
  ↓
确定性地将所有符合准入条件且已有 prepared 的项目晋升为执行周安全基线
  ↓
结算上一周，并为下一预备周创建 UserWeekCapacity
  ↓
排除 paused 项目，并根据优先级、截止风险和 active 项目最低维持量生成不可变 UserWeekAllocationSet；容量不足的低优先级项目允许产生 0 分钟 unfunded item
  ↓
确定性地只为满足 `status=active && budget_minutes>0 && 非 deadline-week && 非 calibration_hold` 的项目创建或复用绑定 allocation item 的空 prepared WeekPlan；其他项目跳过
  ↓
仅符合准入条件的项目在自己的预算内异步执行 WeeklyReview
  ↓
白名单命令校验
  ↓
...
安全基线晋升不依赖 AI，也不依赖用户确认。AI 失败时已排入容量的普通项目继续执行已经晋升的基线；unfunded、paused、deadline-week 和 calibration-hold 项目允许没有新任务，这是明确业务状态而不是规划故障。

项目 Worker 只能在 AllocationSet 和 prepared 壳计划事务提交后并行运行，因此 `active && budget_minutes>0 && 非 deadline-week && 非 calibration_hold` 项目的 Snapshot 中 `week.prepared` 必然存在。unfunded 不投递 Worker；paused 不进入分配；deadline-week 和 calibration-hold 的 prepared 合法缺省且不得生成后续窗口任务。event_exclusive 与 date_inclusive 只决定截止当天是否可执行。应用命令时必须锁定相应 `UserWeekAllocation`，并校验项目任务总量和用户周总量。某个项目失败不会回滚其他项目；其预算保持未使用，除非重新执行容量分配。

截止型目标还必须通过累计截止校验。对每个 cutoff，后端计算 `floor(allocatable_minutes × week_start 至 cutoff 的有效日数 / 通常可用日数) - consumed_to_date`，并取不小于 0 的结果作为可新增容量。单任务已消耗量优先使用累计 actual_minutes；缺失且任务曾完成时回退 estimated_minutes，否则为 0。未完成任务的待安排量不新增持久化字段，而是派生为 `max(0, estimated_minutes - coalesce(actual_minutes, 0))`；完成后为 0。当前周只接受不早于业务日期的 cutoff，预备周 consumed_to_date 为 0，因此不需要每日工时归属。校验所有项目现有未完成与候选新增中 `effective_due_date <= cutoff` 的 required 任务剩余预计分钟不超过该容量。每个任务完整归属一个 effective_due_date，跨截止点工作必须拆分；考试类不计 target_date 当天，交付类可计当天，不引入每日任务计划。

待确认 Proposal 不阻塞执行周。Proposal 到期、依赖版本变化、Project closed 或相关状态改变后自动失效。

### 7.3 日常任务与反馈

任务完成、跳过、恢复或取消同步写入 `TaskEvent` 并立即返回，不调用 AI。普通热路径只更新任务状态、累计实际耗时、revision、周汇总和确定性截止风险；只有截止风险真正触发或用户另行提交反馈时才创建规划运行。周末仍有价值的 planned 任务由周评估写入 `deferred`，并在新周创建带 origin_task_id 的新任务。事件在锁定 Project 的事务内递增项目级 `task_event_revision`，数据库 identity 序号只用于存储和分页，不作为快照一致性水位。

客户端不提供直接创建 Task 的接口。用户提出新增行动时先保存为反馈或对话输入，只有规划工作流通过容量、截止和路线校验后才能创建任务。

用户主动反馈先经过 FeedbackUnderstanding 路由：

1. 同步保存反馈；
2. 分类 task/project/user/goal 范围、immediate/weekly/trend 紧急度，以及是否阻塞当前执行；
3. 明确执行事实转 TaskEvent；当前任务明确被阻塞时进入 temporary；轻微但可继续的感受进入 observe；用户级容量变化先重分配；结构性和目标级变化走各自确认；
4. 目标级分类和变更必须由用户确认；
...
复杂机制采用触发式启用：needs_calibration 只由“有执行机会但整周无用户信号”触发；EventDrivenReplanning 只由明确阻塞、重大事件或截止风险触发；RemainingRouteCalibration 只由确认后的结构性信号触发；GoalChange 只由明确目标级意图触发；跨 Stage 调整只在用户确认改变长期路线时启用。普通 TaskEvent 不经过这些工作流。
...
PlanningRun 使用永久幂等：idempotency_key 即使对应 cancelled 运行也不复用。需要基于新输入重建时，在同一 run_family_key 下递增 generation，并将触发 revision 纳入新键；重试同一 generation 返回原运行。
...
- 将所有符合准入条件且已有 prepared 的项目确定性晋升为执行周安全基线；
- 创建下一目标周的容量记录和项目预算；
- 创建不可变的 AllocationSet 版本及各项目 allocation item；
- 仅为 `active && budget_minutes>0 && 非 deadline-week && 非 calibration_hold` 项目创建或复用绑定预算条目的 prepared WeekPlan 壳；
- 更新 `UserWeekRun` 状态；
...
- `project_revision`：目标、截止、项目优先级及其他项目元数据；
- `preference_revision`：用户总容量和长期稳定偏好；
...
- 项目目标、截止或优先级变化检查 `project_revision`；
- 重新分配容量检查 `preference_revision`，并校验各 allocation item 的 project_revision_snapshot；
...
- actual_minutes 累计覆盖、重复事件幂等、reopened 保留投入和部分投入延期；
- TaskDependency 无环与动态阻塞计算；
- Project.project_revision 与 preference_revision 分离、优先级变更的 AllocationSet 重建；
- 零预算 unfunded 项目不创建 WeekPlan/PlanningRun；暂停项目以单一重分配事务释放当前周未锁定预算和后续预留预算、supersede 当前及后续窗口计划并取消未开始任务；
- 无记录周的 needs_calibration、calibration_hold 与用户恢复分支；
- FeedbackUnderstanding 对完成事实、当前阻塞、轻微困难、用户级容量和目标变化的分流；
- GoalChange 影响预览、用户确认、版本冲突和未来计划失效；
