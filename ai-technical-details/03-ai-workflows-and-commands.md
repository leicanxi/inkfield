...
`prepared_week` 通常必有；任意 Project.target_date 落在当前执行周的 terminal deadline-week 项目，以及 needs_calibration 的待校准项目允许为 null，并在 Snapshot 中显式写 `terminal_window=true` 或 `calibration_hold=true`。前者禁止模型创建或延期任务到项目截止后，后者禁止模型生成新任务，等待用户校准。event_exclusive 与 date_inclusive 只决定截止当天是否可执行。
...
`CreateWeekPlan` 仅用于用户确认后原子应用 InitialPlanning 的初始窗口。常规情形为当前执行周与下一预备周；若本周剩余少于 4 天，则为本周 1—2 个启动任务、下一完整执行周及后一预备周创建三个计划，且必须已有对应周的容量与 allocation item。WeeklyReview、EventDrivenReplanning 和恢复流程不得提出该命令；这些流程使用容量分配事务已经确定性创建并绑定 allocation item 的 `week.prepared` 壳计划。
...
`GoalChange` 不是模型白名单输出，而是后端生成的 `proposal_kind=system_goal_change/system_preference_change` Proposal。系统先创建 `workflow_type=goal_change_candidate` 的 PlanningRun 和 ProposalSet；该运行可以不产生 ModelInvocation。`candidate_id` 等于 PlanningProposal.id；候选通过 preview 展示原值、新值、影响的周、容量变化、截止可行性和是否需要重建路线，expected revisions 存在 Proposal.input_versions。过期、拒绝、版本失效和应用均复用 Proposal/ProposalDecision 生命周期，不维护第二套候选状态。

项目级 payload：

```json
{
  "command_type": "ApplyProjectGoalChange",
  "expected_project_revision": 6,
  "changes": {
    "description": "…",
    "target_date": "2026-09-01",
    "deadline_day_policy": "date_inclusive",
    "priority": 80
  },
  "impact": {
...
- 目标描述、target_date、deadline_day_policy、priority 等项目元数据变化递增 `project_revision`；影响路线时递增 `route_revision`，影响两周任务窗口时递增 `plan_revision`；
- priority 或 target_date 变化必须重新生成受影响周的 AllocationSet/项目分配快照，并校验截止容量；
- 长期容量和稳定偏好变化使用独立的 `ApplyPreferenceChange`，校验 `preference_revision`，不得混入 Project 变更；
- 确认事务按用户周容量 → AllocationSet → allocations → Project → WeekPlan/Task 的统一锁顺序执行，任一版本不符则返回冲突；
- closed Project 不允许修改为继续执行，用户继续目标时创建带 predecessor_project_id 的继任项目；
- 应用后旧 Snapshot、未应用 Proposal 和受影响未来任务计划失效，但已完成任务和冻结路线历史不变。
...
| ApplyProjectGoalChange | 无 | 用户确认后的目标/截止/优先级变更 | 跨 Stage 重建或不可达目标 |
...
- 触发：用户确认目标级信息；
- 输入：目标、当前及必要未来周的容量分配、当前基础、材料摘要；
- 输出：CreateStage、CreateMilestone、CreateWeekPlan、CreateTask；
- 约束：常规只生成当前残周/执行周和下一预备周任务；本周剩余少于 4 天时，本周最多生成 1—2 个启动任务，并额外生成下一完整执行周和后一预备周任务；
- 权限：整个初始批次由用户确认后应用。

### 8.3 WeeklyReview

- 触发：UserWeekRun 完成基线晋升和预算分配；
- 前置：仅 `Project.status=active && budget_minutes>0 && 非 deadline-week && 非 calibration_hold` 的项目允许进入 WeeklyReview；容量协调事务已创建或复用绑定 allocation item 的空 `week.prepared`；unfunded 与 paused 项目不创建 PlanningRun；
...
- 禁止：目标级变化、跨 Stage 路线重建；
 ...
 - 无用户执行信号时：写入 needs_calibration，不推进里程碑、不创建更远 prepared 任务；已有 prepared 仍可晋升为当前安全基线；
- 失败：保留安全基线并重试。
...
- paused 项目不进入本工作流；用户恢复项目后先重新分配容量，再根据恢复时上下文决定 temporary 或 structural；
...
### 8.7 CalibrationRecovery

- 触发：用户返回并回答“暂停了 / 完成但忘记记录 / 计划不合适”；
- `完成但忘记记录`：补写经过确认的 TaskEvent/累计 actual_minutes，再重新运行周评估；
- `暂停了`：暂停项目，不补造任务；
- `计划不合适`：进入 EventDrivenReplanning，允许调整负荷、延期或重建两周窗口；
- 约束：恢复前只使用用户确认的事实，不把无记录推断成完成或失败。

### 8.8 FeedbackUnderstanding

- 触发：自然语言反馈；
- 普通 TaskEvent 不经过本工作流；只有自然语言或表单反馈进入分类；
- 输出：candidate_scope、impact_scope(task/project/user/goal)、impact_nature(temporary/observe/structural/goal_change)、urgency(immediate/weekly/trend)、execution_blocked、signal_kind、结构化候选值、置信度、是否需确认；
- 路由：明确完成/跳过/耗时写 TaskEvent；task + execution_blocked=true 立即进入 EventDrivenReplanning temporary；task + 可继续的轻微困难进入 observe；连续结构性困难进入 structural；user scope 先做容量重分配；goal scope 进入 GoalChange；
- 命令：无；
- 规则：goal_change 无论置信度多高都不能自动生效；任意有效反馈都算执行信号，不触发 needs_calibration。

### 8.9 MaterialUnderstanding
