...
- `paused` 项目绝不获得预算、生成任务或投递规划运行；需要维持轻量习惯时项目必须保持 active 并使用较低预算；
...
- unfunded：active 项目因用户周容量不足获得 0 分钟；不创建 WeekPlan 或 PlanningRun，界面显示“本周未排入容量”；
...
stateDiagram-v2
    ...
    active --> superseded: 重大重规划且用户确认，或项目暂停/终止
    settled --> [*]
    superseded --> [*]
```
...
6. 对没有 prepared 计划且预算大于 0 的 active 项目：普通项目创建空安全基线并记录异常；已有 `needs_calibration` 的项目创建 `creation_source=calibration_hold` 的空 active 基线，不投递任务生成；unfunded 和 paused 项目直接跳过；
7. 只有 `active && budget_minutes>0 && 非 deadline-week && 非 calibration_hold` 的项目，提交后才投递 AI 周评估。

创建空安全基线只防止状态缺口，不生成假任务。普通项目显示“本周计划正在补充”并优先进入 planning_weekly 队列；calibration_hold 项目显示“计划待校准”，等待用户回答，不自动继续滚动。

active WeekPlan 被普通重规划 superseded 时，替代 active WeekPlan 必须在同一事务创建；同一项目同一周不能出现没有当前计划或两个当前计划的中间提交状态。项目暂停或终止是例外：旧计划在同一原子事务中 superseded 后不创建替代计划，因为该项目已不再具有执行资格。
...
- 新 prepared WeekPlan 可以晚于周一生成，但 active WeekPlan 始终存在；needs_calibration 项目不得创建更远 prepared 计划。
- 分配事务只为满足 `status=active && budget_minutes>0 && 非 deadline-week && 非 calibration_hold` 的项目确定性创建空 prepared 壳计划，再投递项目 PlanningRun；WeeklyReview 只能填充这个已存在的计划，Snapshot 构建前必须能解析 `week.prepared`。unfunded、paused 和其他特殊分支的 `week.prepared` 合法缺省且不得创建后续任务。
...
stateDiagram-v2
    [*] --> planned
    planned --> completed: 用户完成（首次完成写 first_completed_at）
...
- Task.actual_minutes 是累计总量投影，完成或 duration_recorded 事件后写入最新值；reopened 不清空 first_completed_at 或已记录投入；
...
- 任意周切换中，每个“本周已排入容量”的 active 项目（current allocation budget_minutes>0 且非 unfunded）恰有一个 active WeekPlan；unfunded 与 paused 项目不要求也不得伪造 WeekPlan；
...
- 项目暂停后不存在其 current active 或后续 reserved allocation、active/prepared WeekPlan、可提交 PlanningRun 或待确认 Proposal；已完成任务与实际投入保持可审计，重分配不会回收这些锁定量；
- published Outbox 与 succeeded PlanningRun 不被混为一个状态；
- 任一失败路径都不会让用户周 planned_minutes 超过 allocatable_minutes。
- 有计划执行机会但整周没有用户任务事件或相关反馈时，ProjectWeekAssessment 必须为 needs_calibration；unfunded 周写 not_scheduled，不得误判为无记录。两种情况都不得推进主里程碑或连续创建更远周计划。
